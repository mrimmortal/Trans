...
- If a requested change touches audio, WebSocket messages, wrapper domains, commands, editor insertion, or persistence, update the graph docs in the same change.
- Do not invent alternate endpoints. The backend WebSocket endpoint is `/ws/audio`.
- Do not duplicate core transcription, wrapper-domain, command, or snippet logic in new places.
- Do not add domain-specific defaults to the vanilla template unless the user asks for a wrapper implementation.
