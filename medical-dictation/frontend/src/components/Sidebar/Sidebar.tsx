...
// ✅ FIX: import Macro type instead of using `any[]`
import { Macro } from '@/types';

interface SidebarProps {
  // ✅ FIX: was `macros: any[]` — now properly typed
  macros: Macro[];
  sessions: Session[];
    ...
    <>
      <aside
        className={`sidebar fixed lg:static w-72 h-full bg-white border-r border-gray-200 flex flex-col overflow-hidden transition-transform z-40 lg:z-auto ${
          isMobileOpen ? 'translate-x-0 sidebar-mobile-open' : '-translate-x-full lg:translate-x-0'
        }`}
        role="complementary"
