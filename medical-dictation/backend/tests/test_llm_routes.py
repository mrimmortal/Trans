...
class LLMRoutesTests(unittest.TestCase):
    ...
    def test_missing_config_returns_503(self):
        with patch("app.api.llm_routes.LMStudioClient") as client_class:
            client_class.return_value.respond.side_effect = LMStudioConfigError("missing config")
    ...
    def test_unavailable_client_returns_502(self):
        with patch("app.api.llm_routes.LMStudioClient") as client_class:
            client_class.return_value.respond.side_effect = LMStudioUnavailableError("offline")
    ...
    def test_successful_call_returns_lmstudio_response(self):
        with patch("app.api.llm_routes.LMStudioClient") as client_class:
            client_class.return_value.respond.return_value = LLMRespondResponse(
                response="Hello back.",
                model="local-model",
            )
