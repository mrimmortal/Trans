...
class TTSRoutesTests(unittest.TestCase):
    ...
    def test_config_error_returns_503(self):
        with patch("app.api.tts_routes.TTSService") as service_class:
            service_class.return_value.synthesize.side_effect = SupertonicConfigError("supertonic is not installed")
    ...
    def test_synthesis_error_returns_502(self):
        with patch("app.api.tts_routes.TTSService") as service_class:
            service_class.return_value.synthesize.side_effect = SupertonicSynthesisError("model download failed")
    ...
    def test_success_returns_wav_audio(self):
        with patch("app.api.tts_routes.TTSService") as service_class:
            service_class.return_value.synthesize.return_value = b"RIFFfake-wav-bytes"
        ...
        service_class.return_value.synthesize.assert_called_once_with("hello", voice="F1", lang="es")
