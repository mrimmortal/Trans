...
def parse_args(argv=None):
    settings_defaults = ServerSettings()
    parser = argparse.ArgumentParser(description="CoreSTT FastAPI browser streaming server")
    parser.add_argument("--host", default=settings_defaults.host)
    parser.add_argument("--port", type=int, default=settings_defaults.port)
    parser.add_argument(
        "--profile",
        "--tuning-profile",
        dest="tuning_profile",
        choices=sorted(TUNING_PROFILES),
        default=settings_defaults.tuning_profile,
        help="Named tuning profile. Parakeet profiles tune cadence/batching/VAD timing, not Whisper beam search.",
    )
    parser.add_argument("--model", default=settings_defaults.model)
    parser.add_argument("--realtime-model", default=settings_defaults.realtime_model)
    parser.add_argument("--language", default=settings_defaults.language)
    parser.add_argument("--engine", "--transcription-engine", dest="transcription_engine", default=settings_defaults.transcription_engine)
    parser.add_argument("--realtime-engine", "--realtime-transcription-engine", dest="realtime_transcription_engine", default=settings_defaults.realtime_transcription_engine)
    ...
    parser.add_argument("--domain-profiles-path", default=settings_defaults.domain_profiles_path)
    parser.add_argument("--default-domain", default=settings_defaults.default_domain)
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=settings_defaults.single_gpu_inference_gate,
    )
    parser.add_argument("--device", default=settings_defaults.device)
    parser.add_argument("--gpu-device-index", type=int, default=settings_defaults.gpu_device_index)
    ...
    parser.add_argument("--realtime-callback", choices=("update", "stabilized"), default=settings_defaults.realtime_callback)
    parser.add_argument("--min-length-of-recording", type=float)
    parser.add_argument("--min-gap-between-recordings", type=float, default=settings_defaults.min_gap_between_recordings)
    parser.add_argument("--post-speech-silence-duration", type=float)
    parser.add_argument("--silero-sensitivity", type=float, default=settings_defaults.silero_sensitivity)
    parser.add_argument("--webrtc-sensitivity", type=int, default=settings_defaults.webrtc_sensitivity)
    parser.add_argument("--realtime-processing-pause", type=float)
    parser.add_argument(
        "--realtime-transcription",
        action=argparse.BooleanOptionalAction,
        default=settings_defaults.realtime_transcription_enabled,
    )
    parser.add_argument("--realtime-use-syllable-boundaries", action="store_true")
    parser.add_argument("--realtime-boundary-detector-sensitivity", type=float, default=settings_defaults.realtime_boundary_detector_sensitivity)
    parser.add_argument("--realtime-boundary-followup-delays", default=",".join(str(value) for value in settings_defaults.realtime_boundary_followup_delays))
    parser.add_argument("--early-transcription-on-silence", type=float)
    parser.add_argument("--initial-prompt")
    parser.add_argument("--initial-prompt-realtime")
    parser.add_argument("--wakeword-backend", default=settings_defaults.wakeword_backend)
    parser.add_argument("--openwakeword-model-paths", default=settings_defaults.openwakeword_model_paths)
    parser.add_argument("--openwakeword-inference-framework", default=settings_defaults.openwakeword_inference_framework)
    parser.add_argument("--wake-words", default=settings_defaults.wake_words)
    parser.add_argument("--wake-words-sensitivity", type=float, default=settings_defaults.wake_words_sensitivity)
    parser.add_argument("--wake-word-activation-delay", type=float, default=settings_defaults.wake_word_activation_delay)
    parser.add_argument("--wake-word-timeout", type=float, default=settings_defaults.wake_word_timeout)
    parser.add_argument("--wake-word-buffer-duration", type=float, default=settings_defaults.wake_word_buffer_duration)
    parser.add_argument("--wake-word-followup-window", type=float, default=settings_defaults.wake_word_followup_window)
    ...
    parser.add_argument("--audio-queue-size", type=int, default=settings_defaults.audio_queue_size)
    parser.add_argument("--max-audio-packet-bytes", type=int, default=settings_defaults.max_audio_packet_bytes)
    parser.add_argument("--max-sessions", type=int, default=settings_defaults.max_sessions)
    parser.add_argument("--max-active-speakers", type=int, default=settings_defaults.max_active_speakers)
    parser.add_argument("--max-audio-queue-seconds-per-session", type=float, default=settings_defaults.max_audio_queue_seconds_per_session)
    parser.add_argument("--pre-recording-buffer-duration", type=float, default=settings_defaults.pre_recording_buffer_duration)
    parser.add_argument("--max-realtime-queue-age-ms", type=int, default=settings_defaults.max_realtime_queue_age_ms)
    ...
    parser.add_argument("--log-level", default=settings_defaults.log_level)
