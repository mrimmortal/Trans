"""
Build and install Kroko-ONNX for the active CoreSTT environment.
"""
...
def parse_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Platform note:\n"
            "  CoreSTT core supports Python 3.11+.\n"
...
def default_work_dir():
    ...
    if os.name == "nt":
        ...
        if root:
            return Path(root) / "CoreSTT" / "kroko-builder"
    elif sys.platform == "darwin":
        return Path.home() / "Library" / "Caches" / "CoreSTT" / "kroko-builder"
    else:
        root = os.environ.get("XDG_CACHE_HOME")
        if root:
            return Path(root) / "corestt" / "kroko-builder"
    return Path(tempfile.gettempdir()) / "corestt-kroko-builder"
...
def ensure_work_dir_writable(work_dir):
    ...
    try:
        ...
        probe = work_dir / ".corestt-kroko-write-test"
...
def ensure_windows_host():
    ...
    if sys.version_info[:2] != (3, 12):
        raise KrokoInstallError(
            ...
            "CoreSTT core supports Python 3.11+, but this Kroko Windows "
            "builder path does not.\n"
            "Create and activate a Python 3.12 x64 environment, install "
            "CoreSTT[kroko-builder] there, then rerun:\n"
